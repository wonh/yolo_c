...
def get_batch_statistics(outputs, targets, iou_threshold):
    ...
    print("get batch length:",targets.shape,len(outputs))
...
def non_max_suppression2(prediction, conf_thres=0.5, nms_thres=0.4):
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        # detections ==》 (more_than_conf_thres_num, 7) 7 =》x,y,w,h,conf,class_conf,class_pred
        detections = torch.cat((image_pred[:, :5], class_confs.float(), class_preds.float()), 1)
        bbox = detections[:,:4].data
        class_score = class_confs.squeeze().float()
        # print(class_score.shape, bbox.shape, class_score)
        # Perform non-maximum suppression
        if detections.size(0) > 1:
            keep = nms(bbox, class_score, nms_thres)
        # if len(keep):
            # 每张图片的最终预测框有pred_boxes_num个，output[image_i]的shape：
            # (pred_boxes_num, 7) 7 =》x,y,w,h,conf,class_conf,class_pred
            output[image_i] = detections[keep.view(-1).long()]
        else:
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    # print("build f: ",target.shape)
    # print("before",target[:5,:24])
    ...
    for i in range(len(target)):
        ...
        boxes[:, 0] = i
        boxes = boxes[boxes[:, 1:].sum(dim=1) != 0]
        new_target.append(boxes)
    target = torch.cat(new_target, 0).cuda()
    ...
    for i, anchor_ious in enumerate(ious.t()):
        # print(i,len(b),len(gj),anchor_ious,len(target[:, :2]))
        # print(nB, b)
